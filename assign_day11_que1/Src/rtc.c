...
{
	uint16_t uints = bin%10;
	  uint16_t tens = (bin/10)%10;
	  uint16_t bcd=(tens<<4)|uints;
	  return bcd;
}
...
void RTC_SetDate(RTC_Date *dt)
{
  ...
  uint16_t mon = Bcd2Bin(dt->Month);
  uint16_t yr = Bcd2Bin(dt->Year);
  uint16_t day = Bcd2Bin(dt->WeekDay);

  uint16_t dr = ((uint32_t)date<<RTC_DR_DU_Pos)
...
void RTC_SetTime(RTC_Time*tm)
{
 ...
 uint16_t min =Bcd2Bin(tm->Minute);
 uint16_t sec = Bin2Bcd(tm->Second);

 uint16_t tr = ((uint32_t)hr<<RTC_TR_HU_Pos)
